 ...
 * @private
 * @constructor
 * @see {@link log.filter#LevelFilter}
 */
function LevelFilter (levels, settings) {
  Filter.call(this);

  levels = levels || [];
  levels = utils.flatten(Array.isArray(levels) ? levels : [ levels ]);
  levels.forEach((level) => {
    utils.guard(TypeError, level != null && level instanceof Level,
...
utils.extend(LevelFilter.prototype, {
  /**
   * @public
   * @function
   * @memberof log.filter
   ...
   */
  addLevel (level) {
    level = utils.flatten(Array.isArray(level) ? level : [ level ]);
    level.forEach((current) => {
      utils.guard(TypeError, current != null && current instanceof Level,
